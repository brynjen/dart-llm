/// Live statistics for a single Ollama instance within an [OllamaPool].
class OllamaInstanceStats {
  const OllamaInstanceStats({
  ...
  @override
  String toString() =>
      'OllamaInstanceStats($baseUrl: $activeConcurrent/$maxConcurrent active, '
      '$queuedRequests queued, ${healthy ? "healthy" : "unhealthy"})';
}

/// Aggregate live statistics for an [OllamaPool].
class OllamaPoolStats {
  const OllamaPoolStats({required this.instances});

  /// Per-instance statistics.
  final List<OllamaInstanceStats> instances;
  ...
  @override
  String toString() =>
      'OllamaPoolStats(${instances.length} instances, '
