...
### Fixed
- macOS, Linux and Windows builds bundled only `libllama`, leaving out the `ggml` libraries it links against. llama.cpp is configured with `-DBUILD_SHARED_LIBS=ON`, so a macOS build also produces `libggml`, `libggml-base`, `libggml-cpu`, `libggml-metal` and `libggml-blas`; only Android collected its whole set. A macOS app appeared to work on the machine that built it purely because the missing libraries were still reachable through llama.cpp's absolute build-tree `LC_RPATH`, and broke as soon as the app was moved or built elsewhere. The build hook now collects the libraries built alongside the primary one on every platform, in a deterministic order. Verified by running inference from a relocated `.app` with the build tree deleted.
- The macOS Flutter loader opened `libllama.dylib`, which is never in the app bundle: Flutter installs each code asset as its own framework, stripping the `lib` prefix and `.dylib` extension, so the library is `llama.framework/llama`. It now opens `@rpath/llama.framework/llama` and falls back to the plain name for non-Flutter embeddings.
