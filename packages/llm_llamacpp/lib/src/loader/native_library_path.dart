...
typedef DladdrNative = ffi.Int Function(
  ffi.Pointer<ffi.Void> addr,
  ffi.Pointer<DlInfo> info,
);
typedef DladdrDart = int Function(
  ffi.Pointer<ffi.Void> addr,
  ffi.Pointer<DlInfo> info,
);
    ...
    try {
      ...
      if (result != 0 && dlInfoPtr.ref.dli_fname.address != 0) {
        ...
        if (fullPath.contains('.apk!')) {
          // ignore: avoid_print
          print('[llm_llamacpp] ERROR: Native libraries are loaded from APK, not extracted to filesystem.');
          // ignore: avoid_print
          print('[llm_llamacpp] Library path: $fullPath');
          // ignore: avoid_print
          print('[llm_llamacpp] To fix this, add android:extractNativeLibs="true" to your AndroidManifest.xml <application> element.');
          // ignore: avoid_print
          print('[llm_llamacpp] This is required for ggml_backend_load_all_from_path() to find backend .so files.');
          return null;
        }
