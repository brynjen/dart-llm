...
class BackendInitializer {
  ...
  static (ffi.DynamicLibrary, LlamaBindings) initializeBackendForIsolate() {
    // ignore: avoid_print
    print('[llm_llamacpp] Initializing backend for isolate (minimal loading)...');
  ...
  static bool _loadBackendsAndroid(ffi.DynamicLibrary lib) {
    ...
    if (nativeLibDir == null) {
      // ignore: avoid_print
      print('[llm_llamacpp] WARNING: Could not determine native library directory on Android');
      // ignore: avoid_print
      print('[llm_llamacpp] Falling back to ggml_backend_load_all() which may fail due to SELinux');
      ...
      try {
        ...
        ggmlBackendLoadAll();
        // ignore: avoid_print
        print('[llm_llamacpp] Called ggml_backend_load_all() - backends might be loaded');
        return true;
      } catch (e) {
        // ignore: avoid_print
    ...
    try {
      ...
      try {
        ...
        // ignore: avoid_print
        print('[llm_llamacpp] Called ggml_backend_load_all_from_path("$nativeLibDir")');
      } finally {
        calloc.free(pathPtr);
      }
    } catch (e) {
  ...
  static bool _loadBackendsManually(ffi.DynamicLibrary lib, String nativeLibDir) {
    // ignore: avoid_print
    print('[llm_llamacpp] Attempting manual backend loading from: $nativeLibDir');
    
    // Get the ggml_backend_load function
    ffi.Pointer<ffi.Void> Function(ffi.Pointer<ffi.Char>) ggmlBackendLoad;
    try {
      ggmlBackendLoad = lib.lookupFunction<
          ffi.Pointer<ffi.Void> Function(ffi.Pointer<ffi.Char>),
          ffi.Pointer<ffi.Void> Function(ffi.Pointer<ffi.Char>)>('ggml_backend_load');
    } catch (e) {
      // ignore: avoid_print
    ...
    if (!dir.existsSync()) {
      // ignore: avoid_print
      print('[llm_llamacpp] ERROR: Native library directory does not exist: $nativeLibDir');
      return false;
    }
    ...
    try {
      ...
      // ignore: avoid_print
      print('[llm_llamacpp] Found ${files.length} files in native lib directory');
          ...
          if (filename.startsWith('libggml-cpu-') && filename.endsWith('.so')) {
            ...
            try {
              ...
              if (result.address != 0) {
                ...
                // ignore: avoid_print
                print('[llm_llamacpp] Failed to load: $filename (returned null)');
              }
            } finally {
              calloc.free(pathPtr);
            }
