...
void main() {
  group('VLLM Integration Tests - Tool Calling', () {
    ...
    group('Finish reason', () {
      test(
        'a named tool_choice turn is reported as a tool-call turn',
        () async {
          // vLLM answers a named `tool_choice` with `finish_reason: "stop"`
          // while returning a complete, valid call — deterministically, and
          // in non-streaming mode too. The converter used to match only
          // `tool_calls`, so the call was dropped and the round reached the
          // caller with no content and no calls, indistinguishable from the
          // model idling. Per the OpenAI specification a turn that called a
          // tool finishes as `tool_calls`, whatever the server spelled.
          ...
          expect(
            completed,
            isNotNull,
            reason: 'a complete call must survive a stop finish',
          );
          expect(completed!.name, 'get_weather');
          expect(
            () => completed!.argumentsJson,
            returnsNormally,
            reason: 'the arguments must decode to an executable object',
          );
