...
void main() {
  group('VLLM Integration Tests - Embeddings', () {
    late VLLMChatRepository repo;

    setUp(() {
      repo = createRepository(customBaseUrl: embeddingBaseUrl);
    });
    ...
    group('Embedding Tests', () {
      ...
      test(
        'batchEmbed with configured embedding model returns embeddings',
        () async {
          final texts = [
          ...
          for (var i = 0; i < embeddings.length; i++) {
            ...
            expect(embeddings[i].model, equals(embeddingModel));
  ...
  }, skip: embeddingTestsEnabled ? false : 'Set VLLM_ENABLE_EMBEDDING_TESTS=true');
}
